...
def crop_edges(
    ...
    **kwargs,
    ...
    crop_range : {int | float | str} or tuple of {ints | floats | strs}
        ...
        converted to indices. If str HyperSpy fancy indexing is used
        (e.g. ``rel0.1`` will crop 10% on each side, or ``100 nm`` will
        crop 100 nm on each side). If a number or a tuple of size 1 is
        passed, all sides are cropped by the same amount. If a tuple of 
        size 2 is passed (``crop_x``, ``crop_y``), a different
    ...
    rebin_nav : bool
        If the navigation axes shape is different between signals in 
        the list S, all signals will be rebinned to match the shape of
        the first signal in the list. Note this does not take into account
        the calibration values of the navigation axes.
    kwrgs
    ...
    S_cropped : Signal or list of Signals
        A list of smaller, cropped Signal objects or a cropped single 
        Signal if only one signal object is passed as input.
    """
